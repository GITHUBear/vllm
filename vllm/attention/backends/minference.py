...
def Minference_prefill(
    query_states: torch.Tensor,  # bhnd
    key_states: torch.Tensor,
    value_states: torch.Tensor,
    vertical_slash_config: list,
    ...
    for head in range(query_states.size(1)):
        ...
        output[:, head : head + 1, :, :] = vertical_slash_sparse_attention(q, k, v, vertical_topk, slash_topk)
